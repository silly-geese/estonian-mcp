...
- **`paradigm` led with the wrong variant of a slot.** Estonian builds
  its oblique plural cases on the genitive plural, and Vabamorf returns
  a slot's variants in lexicon order, so `raamat` came back as
  `pl all: raamatuile, raamatutele`. `raamatute` is the genitive plural,
  so `raamatutele` is the form this paradigm builds on its own stem;
  `raamatuile` is the i-plural, a real but literary formation built on a
  different one. A caller takes the first entry, which is the point of
  ordering it, so the table led with the literary form for nine slots of
  an ordinary noun.

  Variants are now ordered by the paradigm's own genitive stem, which is
  grammar rather than statistics: the stem was generated by the same
  paradigm under the same hint, so it cannot belong to another lexeme.
  Measured on 11,011 rows of real Estonian, first-candidate accuracy
  goes from **88.0% to 99.3%**, any-candidate 99.9%.
  `inflection_et` is unchanged at 99.1% / 100% adjudicated, since its
  four cases barely exercise this.

  Corpus frequency was the first attempt and scored 5 points lower. It also promoted homographs, because a surface can be frequent
  as some other word entirely: `käsi pl ad` became `käsil`, a lexicalised
  adverb, over `kätel`, and `soov pl p` became `soovisid`, the past tense
  of `soovima`, over `soove`. Both are pinned as tests now. The stem rule
  needs no model either, so the cheapest tool in the server keeps loading
  nothing: frequency ranking would have consulted the 34 MB vocabulary
  for 92.5% of words against 2% before.
...
- **A second benchmark: `scripts/eval_kaanamiskorpus.py`.**
  ...
  It is pinned to a commit, downloaded at run time, never vendored here,
  so nothing in this repository redistributes CC-BY-SA data and the
  ...
  form versus how often it leads with it. `--limit` samples evenly
